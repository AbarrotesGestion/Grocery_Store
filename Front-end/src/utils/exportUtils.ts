...
export const exportToExcel = (data: any[], fileName = 'reporte') => {
...
// Función para exportar a PDF con la comparativa de 7 días, mensual y últimas ventas
export const exportToPDF = (tituloReporte: string, datosResumen: any, fileName = 'reporte') => {
  ...
  for (let i = 0; i < mesesLabels.length; i++) {
    ...
    if (ventaMes > 0 || datosResumen.mesVentasData) {
  ...
  container.innerHTML = `
    ...
    <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
      ...
      <tbody>
        ${datosResumen.ultimasVentas?.map((v: any, i: number) => `
          <tr style="border-bottom: 1px solid #e5e7eb; background-color: ${i % 2 === 0 ? '#ffffff' : '#f9fafb'};">
        ...
        `).join('') || '<tr><td colspan="5" style="padding: 6px; text-align: center;">Sin registros</td></tr>'}
      </tbody>
    </table>
  `;
