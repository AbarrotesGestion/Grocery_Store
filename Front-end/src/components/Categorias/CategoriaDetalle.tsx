...
export default function CategoriaDetalle() {
  ...
  const [categoria, setCategoria] = useState({
    ...
    productos: [
      { id: 101, nombre: 'Arroz Súper Extra 1kg', stock: 50, precio: 18.00 },
      { id: 102, nombre: 'Frijol Negro 1kg', stock: 40, precio: 26.00 },
    ]
  });

  const [isModalOpen, setIsModalOpen] = useState(false);

  const handleSaveModal = (updatedData: { nombre: string; descripcion: string }) => {
    setCategoria(prev => ({
      ...prev,
  ...
  return (
    <div className="p-6 bg-dark-bg text-gris-calido min-h-screen space-y-6">
      ...
      <CategoriaModal 
        ...
        initialData={{ id: categoria.id, nombre: categoria.nombre, descripcion: categoria.descripcion }}
      />
    </div>
  );
}